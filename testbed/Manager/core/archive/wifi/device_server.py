...
class DeviceServer:
    ...
    def _deviceDisconnected_callback(self, device):
        ...
        self.events.device_disconnected.set(resource=device, flags={'type': device.information.device_type})

    # ------------------------------------------------------------------------------------------------------------------
    def _deviceStream_callback(self, stream, device):
        self.callbacks.stream.call(stream, device)
        self.events.stream.set(resource=stream, flags={'type': stream.type, 'device_id': device.information.device_id})
