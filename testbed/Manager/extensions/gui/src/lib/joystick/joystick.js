...
export class Joystick {
    ...
    setDeadzone(dz = 0.15) {
        this._deadzone = Math.max(0, Math.min(0.95, Number(dz)));
    }
    ...
    async rumble(durationMs = 500, strong = 0.5, weak = 0.5) {
    ...
    /** Info helpers */
    getType() {
        return this._gamepad()?.mapping || "unknown";
    }
    ...
    _tick() {
        ...
        buttonsNow.forEach((b, i) => {
            ...
            if (b.pressed && !p.pressed) {
                this.callbacks.get("button").call({
                    index: i,
                    name,
                    type: "down",
                    value: b.value,
    ...
    _gamepad() {
        return this._pads()[this.index] || null;
    }
    ...
    _axisName(i) {
        for (const [name, idx] of Object.entries(this._mapping.axes)) if (idx === i) return name;
    }

    _buttonName(i) {
        for (const [name, idx] of Object.entries(this._mapping.buttons)) if (idx === i) return name;
    }
...
export class JoystickManager {
    ...
    constructor(opts = {}) {
        ...
        console.log("JoystickManager: starting");
    ...
    rescan() {
        ...
        for (const gp of found) {
            if (!this._known.has(gp.index)) {
                ...
                console.log("JoystickManager: new joystick", info);
    ...
    getByIndex(index) {
        return this._known.get(index) || null;
    }
